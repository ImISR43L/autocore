...
interface SqlModelingWizardProps {
  ...
  <div className="h-full w-full overflow-y-auto scrollbar-thin scrollbar-thumb-zinc-700 scrollbar-track-transparent">
    <div
      className={cn(
        ...
        isWide ? "max-w-[98%] h-full" : "max-w-5xl",
      )}
    >
      {children}
    </div>
...
export function SqlModelingWizard({
  initialValues,
  onSubmit,
}: SqlModelingWizardProps) {
  ...
  const defaults = {
    type: "EXERCISE",
    title: "",
    subject: "SQL_MODELING",
    description: "",
    slug: "",
    classroomId,
    referenceModel: EMPTY_ER_MODEL,
    maxAttempts: 0,
    startDate: "",
    deadline: "",
  };
  ...
  const draftStorageKey = problemIdForDraft
    ? `problem-wizard-draft-sql-modeling-${problemIdForDraft}`
    : "problem-wizard-draft-sql-modeling";
  ...
  const handleFinalSubmit = async (data: ProblemFormValues) => {
    ...
    (data as any).subject = "SQL_MODELING";
    ...
    <div className="h-full flex flex-col relative bg-surface rounded-xl overflow-hidden border border-border shadow-2xl">
      ...
      <FormProvider {...methods}>
        ...
        >
                ...
                <div className="flex flex-col gap-2 flex-1 min-h-[300px]">
                  ...
                  >
                    <MarkdownInput
                      ...
                      placeholder="# Descreva o domínio que o aluno precisa modelar (ex: 'uma biblioteca com livros, autores e empréstimos')..."
                    />
                  </div>
                </div>
              </div>
          ...
          {/* PASSO 2: GABARITO (DER de referência) */}
          <Stepper.Step label="Gabarito" validationFields={[]}>
            <ScrollableStepContent isWide={true}>
              <div className="h-full min-h-[550px] flex flex-col gap-3">
                <div className="flex-none flex items-start gap-2 text-xs text-muted bg-surface/60 border border-border rounded-lg p-3">
                  <Network size={14} className="flex-none mt-0.5 text-primary" />
                  <p>
                    Este diagrama é <strong>opcional</strong> na Fase atual —
                    a correção é manual (o professor avalia o diagrama do
                    aluno via nota/comentário, mesmo fluxo já usado nas
                    outras matérias). Desenhar um gabarito aqui serve como
                    referência visual durante a correção, e vira a base de
                    comparação quando um corretor automático de DER
                    existir.
                  </p>
                </div>
